...
constexpr std::size_t buffer_size(const char (&buffer)[N]) noexcept
{
  return N;
}
...
{
  char commandResponse[5];
  TFMiniPlus::write(ResetCommand, buffer_size(ResetCommand));
  if (TFMiniPlus::readCommandResponse(commandResponse, buffer_size(commandResponse)) && commandResponse[3] == 0x00)
  {
    return true;
  }
  return false;
...
bool TFMiniPlus::setFrameRate(uint16_t framerate)
{
  ...
  char buffer[6] = {CMD_FRAME_MARKER, 0x06, 0x03, newRateLow, newRateHight};
...
bool TFMiniPlus::setMeasurementTo(uint16_t measurement)
{
  ...
  char buffer[5] = {CMD_FRAME_MARKER, 0X05, 0x05, measurementH, measurementL};
...
{
  char commandResponse[5];
  char buffer[5] = {CMD_FRAME_MARKER, 0x05, 0x07};
...
bool TFMiniPlus::validateChecksum(const char dataBuffer[], size_t length)
{
...
void TFMiniPlus::write(const char buffer[], size_t length)
{
...
bool TFMiniPlus::readCommandResponse(char buffer[], size_t buffer_size)
{
  ...
  char response[MAX_CMD_RESPONSE_LENGTH] = {CMD_FRAME_MARKER, length};
...
uint8_t TFMiniPlus::generateChecksum(const char buffer[], size_t length)
{
  uint16_t sum = 0x0000;
  for (uint8_t i = 0; i < length; i++)
  {
...
uint16_t TFMiniPlus::readInt16FromBuffer(const char buffer[], size_t startIndex)
{
  return (uint16_t(buffer[startIndex]) << 8) | uint16_t(buffer[startIndex + 1]);
}

uint32_t TFMiniPlus::readInt32FromBuffer(const char buffer[], size_t startIndex)
{
...
void TFMiniPlus::copyBuffer(char buffer1[], const char buffer2[], size_t buffer_size,
                            size_t length)
{
...
void TFMiniPlus::resetBuffer(char buffer[], size_t length)
{
  for (uint8_t i = 0; i < length; i++)
  {
    buffer[i] = 0x00;
